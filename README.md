...
将视频（B站 / YouTube / GDC Vault / 本地文件）转为**图文并茂的 Markdown**，一键导出截图 + 字幕，适合知识整理、课程笔记、内容归档。
...
- YouTube 视频链接
- GDC Vault 播放页链接（自动解析内嵌播放器和 HLS 视频流）
- 本地视频文件（拖拽）
...
- 支持 `gdcvault.com/play/...` 播放页，程序会自动解析页面中的 Blaze Streaming 播放器和 `.m3u8` 视频流
- 会尽量读取页面标题作为输出文件名；如果页面结构变化导致解析失败，会回退给 yt-dlp 继续尝试
- GDC Vault 通常没有平台字幕，处理时会按需使用 Whisper 转录
